...
### 准备：部署引导（一次性）
```powershell
# 新 agent 首次接手工作区按此两步。后续直接从 init-workspace 开始。
# 1. agent 准备环境：python 3.12+ + uv，缺则下载静默安装
# 2. agent 读 skills/cst-runtime-cli/scripts/bootstrap.py → 写为 bootstrap.py → 部署
uv run python bootstrap.py --skill-path <skill-root>\scripts

# 输出 status=ready 后，初始化工作区（自动注册 CST 路径）：
uv run python -m cst_runtime init-workspace --workspace <path>
```
...
新 agent 首次接手工作区时执行部署引导流程（见"部署引导"章节）。`bootstrap.py` 成功（`status=ready`）后，运行 `init-workspace` 初始化工作区并自动注册 CST 路径。
...
只依靠本 Skill 和 CLI 完成任务。首次使用 bootstrap.py 部署引导；
成功后 init-workspace 初始化工作区并自动注册 CST 路径。
...
全部后续命令使用 uv run python -m cst_runtime ...。
