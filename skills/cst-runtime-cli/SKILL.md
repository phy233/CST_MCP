...
> 运行边界：本 Skill 的所有 CLI 命令均必须由 CST 兼容的 Python 3.9 worker 执行。下文的 `uv run python -m cst_runtime` 是历史写法，不可用于生产；应替换为 `& $env:CST_WORKER_PYTHON -m cst_runtime`，并确保 `PYTHONPATH` 指向 runtime source。`cst-mcp` 则安装在独立的 Python 3.12+ 环境中，只通过 JSON-RPC 调用 worker。
...
`cst-runtime` 不安装到 `.venv`；MCP 通过 `runtime.worker_python` 启动它，并以 stdin/stdout JSON-RPC 通信。
