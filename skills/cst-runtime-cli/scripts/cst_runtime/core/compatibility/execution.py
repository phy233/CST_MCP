...
def resolve_cst_temp_context(
    ...
    *,
    ...
    """实测并缓存当前 CST 可用于 History 的 Temp 路径表达式。"""
    ...
    if cached:
        ...
        # 未知版本不预设语法，只有即时 VBA 实测成功的候选项才允许写入 History。
        ...
        for expression in candidates:
            macro = "\n".join(
                [
                    ...
                    f'Open "{vba_string(str(probe_path))}" For Output As #cstRtProbeFile',
            ...
            try:
                ...
                if not probe_path.is_file():
                    ...
                    continue
                reported = _decode_text(probe_path).strip()
...
__all__ = [
    "execute_immediate_vba",
    "execute_text_query",
    "resolve_cst_temp_context",
    "vba_string",
]
